...
$where = " c.IsDeleted = 'NO' and ((c.CreatorID = :creatorID and c.ParentID is null) || cw.UserID = :creatorID)";
        ...
        <?php
            for($i = 0; $i < count($info); $i++)
            {
                ...
                else
                {
                    ...
                    if(!empty($info[$i]["cwUserID"]))
                        $trString .= "&mode=crowdWorker";
                    $trString .= "'>";
                    $trString .= "<img src='../img/edit-icon.png' title='Change or add to your project' >
                                    </a>
                                </td>";
                    $trString .= "<td style='text-align: center'>
