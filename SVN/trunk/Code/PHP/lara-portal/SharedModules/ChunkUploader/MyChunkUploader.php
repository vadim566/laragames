...
class MyChunkUploader {
	...
	function __construct( $working_dir = null ) {
		...
		$this->_tmp_dir = ! empty( $working_dir ) ? $working_dir : ContentTmpDirectory;
	 ...
	 */
	private function _copy_file( $input_file, $output_file, $sufix = '', $write_mode = 'w' ) {

	    if($sufix == "final")
        {
            $output_file = ContentTmpDirectory . $output_file;
        }
	...
	private function _merge_files() {
		...
		foreach ( $files as $chunk_filename ) {
			$this->_copy_file( $chunk_filename, $this->_filename, 'final', 'a' );
			unlink( $chunk_filename );
		}
