...
function upload_file(file){
	...
	chunk_uploader.on_done=function(fileName){
		b.disabled=false;
		a.disabled=!b.disabled;
		p.style.backgroundColor='#13B048';
		if(document.getElementById("UploadType").value == "Text")
		{
			ImportLaraContent(fileName);
		}
		else if(document.getElementById("UploadType").value == "Lexicon")
		{
			if(document.getElementById("UploadType_resource").checked == true)
			{
				ImportLaraResource(fileName);
			}
			else if(document.getElementById("UploadType_MWE").checked == true)
			{
				ImportLanguageMWE(fileName);
			}
		}
	...
	chunk_uploader.upload_chunked('../SharedModules/ChunkUploader/upload.php',file);
}

function ImportLaraContent(fileName)
{
	showLoading('#importLaraContentDiv');
	var data = { task: 'ImportLaraContent', fileName: fileName};
	$.ajax({
		...
		success: function (response) {
			hideLoading('#importLaraContentDiv');
...
function ImportLaraResource(fileName)
{
	showLoading('#importLaraContentDiv');
	var data = { task: 'ImportLaraResource', fileName: fileName, languageID: $('#LanguageID').val()};
	$.ajax({
		url         : '../data/Content.data.php',
		data        : data,
		type        : 'GET',
...
function ImportLanguageMWE(fileName)
{
	showLoading('#importLaraContentDiv');
	var data = { task: 'ImportLanguageMWE', fileName: fileName, languageID: $('#MweLanguageID').val()};
	$.ajax({
		url         : '../data/Language.data.php',
		data        : data,
		type        : 'GET',
		contentType : 'application/json; charset=utf-8',
		success: function (response) {
