...
def game9(story_name, file=None):
    ...
    while len(bad_match) <= 4:
        rand_i=random.sample(range(0,size_of_story),1)
        bad_split=sentance[rand_i[0]].split(" ", bad_sent_len_size)
        if len(bad_split) >bad_sent_len_size:
            bad_match.append(sentance[rand_i[0]])
...
def generate_game9(story_name,g_number=0,g_wins=0):
    g_number = int(g_number)
    g_wins = int(g_wins)
    t_w,m_s,t_m,b_w0,b_w1,b_w2,b_w3,s_name=game9(story_name)
    if g_number > 9:
        return check_if_finished(g_wins, g_number)
    else:
        return render_template('game9_template.html', t_answer=t_w, question0=m_s, question1=t_m,fake_answer_0=b_w0, fake_answer_1=b_w1, fake_answer_2=b_w2,fake_answer_3=b_w3, name=s_name,  g_number=g_number, wins=g_wins)


def submit_g9(option=0,answer=0,default_value=0,g_number=0,wins=0):
    ...
    return redirect(url_for('app.generate_game9',  values=values_s))